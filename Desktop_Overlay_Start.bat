...
echo.
"%VENV_PY%" -m pip install --require-hashes --only-binary=:all: -r "%~dp0requirements.txt"
if errorlevel 1 (
  echo.
  echo   설치에 실패했습니다. 아래 명령을 직접 실행해 보세요.
  echo     "%VENV_PY%" -m pip install --require-hashes --only-binary=:all: -r "%~dp0requirements.txt"
  echo.
  pause
  exit /b 1
)
