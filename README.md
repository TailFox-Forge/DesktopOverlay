...
기본값은 클릭 통과가 꺼진 상태라 처음 실행 후 바로 드래그로 위치를 잡을 수 있고, 트레이 아이콘에서 모든 설정을 바꿀 수 있습니다.
...
Python 3.10 이상이 필요합니다. 현재 소스 실행과 CI는 Python 3.10 기준으로 검증합니다.
...
```bash
pip install --require-hashes --only-binary=:all: -r requirements-release.txt
pytest
```
...
클릭 통과를 켜면 창을 직접 잡지 않고 아래 프로그램으로 마우스 입력을 넘깁니다. 위치를 다시 조절하려면 트레이 메뉴에서 **클릭 통과**를 끄세요.
...
| 항목 | 기본값 | 이유 |
|---|---|---|
| 클릭 통과 | **꺼짐** | 최초 실행 후 바로 드래그로 위치를 잡기 쉽게 |
...
기본값은 클릭 통과가 꺼져 있으므로 **처음 실행하면 캐릭터를 바로 드래그해 위치를 잡을 수 있습니다.**

아래 프로그램을 클릭해야 하는 상태에서는 트레이 아이콘 → **클릭 통과**를 켜세요. 위치를 다시 옮길 때는 끄면 됩니다.
